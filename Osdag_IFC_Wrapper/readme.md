...
Added a test suite to validate the export logic without needing the full GUI.
It mocks the CAD objects for the 4 target modules and verifies the IFC hierarchy, placements, and property sets.
...
## Dependencies
Requires `ifcopenshell` (needs to be built with OCC support, the pip wheel usually handles this fine on Windows/Linux).
If you get import errors viewing IFCs, just run:
