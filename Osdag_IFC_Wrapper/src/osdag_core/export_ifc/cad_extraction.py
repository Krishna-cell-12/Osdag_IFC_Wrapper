...
def extract_metadata(module_obj, design_dict=None):
    ...
    # Include the CAD class name for axis handler dispatch
    # This is needed by the IFC generator to select the correct
    # local coordinate system handler for each connection module
    cls = getattr(module_obj, '__class__', None)
    meta['CAD_Class'] = cls.__name__ if cls is not None else ''
