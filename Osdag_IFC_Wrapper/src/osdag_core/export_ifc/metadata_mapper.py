...
class MetadataMapper:
    ...
    def assign_material(self, ifc_element, material_name="Steel",
                        grade=""):
        """
        Creates and assigns an IfcMaterial to the given element via
        IfcRelAssociatesMaterial. Essential for Revit BoQ generation.
        """
    ...
    def assign_revit_standard_psets(self, ifc_element, element_type,
                                     osdag_obj):
        """
        Assigns Revit-recognized standard property sets so that
        the element appears correctly in Revit's schedule views.

        :param element_type: 'beam', 'column', 'plate', or 'fastener'
        """
        props = {}

        if element_type in ('beam', 'column'):
            # Standard Pset_BeamCommon / Pset_ColumnCommon
            pset_name = (f"Pset_{element_type.capitalize()}Common")
            designation = str(getattr(osdag_obj, 'designation', ''))
            if designation:
                props['Reference'] = designation
            material = str(getattr(osdag_obj, 'material', 'Steel'))
            if material:
        ...
        elif element_type == 'fastener':
            pset_name = "Pset_MechanicalFastenerCommon"
            grade = str(getattr(osdag_obj, 'property_class',
                                getattr(osdag_obj, 'grade', '')))
            if grade:
                props['Grade'] = grade
            dia = float(getattr(osdag_obj, 'd',
                               getattr(osdag_obj, 'D', 0)))
            if dia > 0:
                props['NominalDiameter'] = dia
        else:
            return

        if props:
            self.assign_standard_pset(ifc_element, pset_name, props)
