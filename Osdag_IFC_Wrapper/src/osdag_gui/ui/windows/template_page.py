...
class CustomWindow(QWidget):
    ...
    def create_menu_bar_items(self):
        ...
        # IFC Export/Import actions
        export_ifc_action = QAction("Export to IFC", self)
        export_ifc_action.setShortcut(QKeySequence("Alt+E"))
        export_ifc_action.triggered.connect(lambda: self.output_dock.export_to_ifc(self.backend))
        file_menu.addAction(export_ifc_action)

        open_ifc_action = QAction("Open IFC File", self)
        open_ifc_action.setShortcut(QKeySequence("Alt+F"))
        open_ifc_action.triggered.connect(lambda: self.display_ifc_model())
    ...
    def display_ifc_model(self, ifc_path=None, main=None):
        """
        Loads an IFC file into the pythonOCC viewer.
        Uses ifcopenshell.geom to convert IFC geometry back to OCC TopoDS_Shapes.
        
        :param ifc_path: Optional path to the .ifc file to display
        :param main: Backend module object (used for design status checks elsewhere)
        """
        if not self._is_display_ready():
            CustomMessageBox(
                title="Warning",
                text="3D display is not ready yet. Please wait for initialization.",
                dialogType=MessageBoxType.Warning
            ).exec()
            return
        ...
        try:
            ...
            CustomMessageBox(
                title="Error",
                text="ifcopenshell is not installed. Please install it to view IFC files.\n\nRun: pip install ifcopenshell",
                dialogType=MessageBoxType.Warning
            ).exec()
            return

        if ifc_path is None:
            ifc_path, _ = QFileDialog.getOpenFileName(
                self, "Open IFC File", 
        ...
        try:
            ...
            # Configure geometry settings for OCC shape generation
            ...
            # Color map for different IFC element types
            from OCC.Core.Quantity import (
                Quantity_Color, Quantity_TOC_RGB
            )
            
            color_map = {
                'IfcBeam': Quantity_Color(0.3, 0.5, 0.8, Quantity_TOC_RGB),
                'IfcColumn': Quantity_Color(0.4, 0.6, 0.9, Quantity_TOC_RGB),
                'IfcPlate': Quantity_Color(0.7, 0.7, 0.3, Quantity_TOC_RGB),
                'IfcFastener': Quantity_Color(0.5, 0.5, 0.5, Quantity_TOC_RGB),
            }
            default_color = Quantity_Color(0.6, 0.6, 0.6, Quantity_TOC_RGB)
            ...
            for product in products:
                if product.Representation is not None:
                    try:
                        ...
                        # Display the shape
                        self.display.DisplayShape(occ_shape, update=False)
                        element_count += 1
                    except Exception as e:
                        print(f"[IFC Viewer] Warning: Could not render "
                              f"{product.Name}: {e}")
                        continue
            ...
            print(f"[IFC Viewer] Successfully displayed {element_count} "
                  f"elements from {os.path.basename(ifc_path)}")
            
        except Exception as e:
            CustomMessageBox(
                title="Error",
                text=f"Failed to load IFC file:\n{str(e)}",
                dialogType=MessageBoxType.Warning
            ).exec()

    def auto_generate_ifc(self, main):
        """
        Automatically generate an IFC file after a successful design.
        Saves to a temporary location and optionally displays it.
        
        :param main: Backend module object
        :returns: Path to generated IFC file, or None
        """
        ...
        try:
            import tempfile
            ifc_dir = os.path.join(tempfile.gettempdir(), 'osdag_ifc')
            os.makedirs(ifc_dir, exist_ok=True)
            
            module_name = main.module_name().replace(' ', '_')
            ifc_path = os.path.join(ifc_dir, f"{module_name}_auto.ifc")
            
            # Delegate to output_dock's export method
            self.output_dock.export_to_ifc(main, ifc_path)
            
            if os.path.exists(ifc_path):
                print(f"[IFC] Auto-generated IFC file: {ifc_path}")
                return ifc_path
            return None
            
        except Exception as e:
            print(f"[IFC] Auto-generation failed: {e}")
            return None
